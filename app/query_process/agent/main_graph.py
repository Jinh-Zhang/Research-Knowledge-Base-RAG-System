...
from app.query_process.nodes.node_search_embedding_hyde import (
...
# 注册所有节点
builder.add_node("node_item_name_confirm", node_item_name_confirm)  # 确认论文标题
...
def route_after_item_confirm(state: QueryGraphState):
    ...
    if state.get("answer"):
        """
        这主要发生在 node_item_name_confirm 节点无法直接确定唯一的论文标题，从而需要“反问用户”或“拒绝回答”的场景。
        具体来说，有以下两种情况会导致 state 中直接出现 answer ，从而跳过后续的检索流程，直接输出：
        1. 多选一（反问用户） ：
        - 场景 ：用户问得太模糊（比如“RAG论文”），系统发现数据库里有多篇相近论文，且置信度都不足以直接确认。
        - 处理 ：节点会生成一条反问句作为 answer ，例如：“您是想问以下哪篇论文：论文A、论文B？请进一步明确标题。”
        - 结果 ：此时不需要再去检索文档了，直接把这句话发给用户让他选。
        2. 查无此人（拒绝回答） ：
        
        - 场景 ：用户问了一个系统里压根没有的论文，或者评分过低（<0.6）。
        - 处理 ：节点会生成一条拒绝句作为 answer ，例如：“抱歉，未找到相关论文，请提供更准确的论文标题或更多上下文。”
        - 结果 ：同样不需要后续检索，直接结束流程。
        """
