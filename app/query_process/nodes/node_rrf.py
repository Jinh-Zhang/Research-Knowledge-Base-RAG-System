...
def node_rrf(state):
    ...
    logger.info(
        f"RRF 输入统计: Embedding源={len(embedding_chunks)}条, HyDE源={len(hyde_embedding_chunks)}条"
    )

    # Debug 日志：打印部分 ID 以便核对
    if embedding_chunks:
        logger.debug(
            f"Embedding源 chunk_ids (前5个): {[c.get('chunk_id') for c in embedding_chunks[:5]]}"
        )
    ...
    rrf_res = reciprocal_rank_fusion(source_weights, k=60, max_results=30)
...
def _as_entity_list(state_list) -> List[Dict[str, Any]]:
    ...
    for doc in state_list or []:
        ...
        if hasattr(doc, "entity") and hasattr(doc, "id"):
            # 1. 提取 entity 中的业务字段 (如 content, paper_title, chunk_id 等)
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(">>> 测试结果摘要:")
        print(
            f"输入数量: Embedding={len(mock_embedding_chunks)}, HyDE={len(mock_hyde_chunks)}"
        )
        ...
        for i, doc in enumerate(rrf_chunks, 1):
            ...
            print(
                f"Rank {i}: ID={doc_id}, Title={doc.get('file_title')}, Content={doc.get('content')[:20]}..."
            )
