...
def node_search_embedding(state):
    """
    核心节点函数：基于已确认论文标题+改写后的用户问题，执行Milvus向量数据库混合检索
    流程：用户问题向量化 → 构造带论文标题过滤的混合搜索请求 → 执行稠密+稀疏混合检索 → 返回检索结果
    ...
    # 对每个论文标题添加双引号，拼接为Milvus支持的in语法格式
    ...
    reqs = create_hybrid_search_requests(
        ...
        expr=expr,  # 论文标题过滤表达式，缩小检索范围（仅检索指定论文的向量）
        limit=20,  # 底层检索返回数量（后续会再过滤为5，预留更多结果做重排序）
    )
